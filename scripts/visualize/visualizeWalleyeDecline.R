...
mutateWallyDecline <- function(filename){
  ...
  all.bass <- xml_add_child(svg, 'g','id'='all-bass','transform'=sprintf("translate(%s,0)", as.numeric(vb[3])+100), class='background-bass')
  all.wally <- xml_add_child(svg, 'g', 'id'='all-walleye','transform'="translate(-100,0)", class='background-walleye')
  n = 30
  set.seed(211)
