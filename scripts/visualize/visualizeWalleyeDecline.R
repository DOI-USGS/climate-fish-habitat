...
mutateWallyDecline <- function(filename, mobile=FALSE, wallyNorm=NULL, bassNorm = NULL){
  ...
  xml_add_child(svg, 'text','Natural walleye reproduction (young/mile)', x=sprintf("%s",as.numeric(vb[1])+5), y='25', 'text-anchor'="begin", id='y-title')
  xml_add_child(svg, 'text','Largemouth bass relative abundance', x=sprintf("%s",as.numeric(vb[3])-5), y='25', 'text-anchor'="end", id='y-title')
